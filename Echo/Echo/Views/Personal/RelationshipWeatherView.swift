...
struct RelationshipWeatherView: View {
    ...
    private var forecastCard: some View {
        ZStack {
            ...
            if weatherType == .stormy { Image(systemName: "bolt.fill").font(.system(size: 24)).foregroundStyle(Color.yellow).offset(x: 100, y: -40).opacity(pulseLightning ? 1 : 0).animation(.easeInOut(duration: 0.15).repeatForever(autoreverses: true).delay(1), value: pulseLightning) }
            VStack(spacing: 8) {
                ...
                HStack { Text("\(temperature)°").font(.system(size: 14, weight: .semibold, design: .rounded)).foregroundStyle(Color.white.opacity(0.9)); Spacer(); Text("\(healthyCount)\u{2600}\u{fe0f}  \(warningCount)\u{26a0}\u{fe0f}  \(criticalCount)\u{1f329}\u{fe0f}").font(.system(size: 13, weight: .medium, design: .rounded)).foregroundStyle(Color.white.opacity(0.9)) }.padding(.horizontal, 20)
            }.padding(.top, 20)
        }.onAppear { animateClouds = true; animateRain = true; pulseLightning = true }
    }
    ...
    private var contactsWeather: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(0..<contacts.count) { i in let c = contacts[i]; let h = AIEngine.healthScore(for: c); let w = personalWeather(health: h.level, gap: c.lastReachedOut ?? Date()); HStack(spacing: 12) { ZStack { Circle().fill(w.color.opacity(0.15)).frame(width: 40, height: 40); Image(systemName: w.icon).font(.system(size: 16)).foregroundStyle(w.color) }; VStack(alignment: .leading, spacing: 2) { Text(c.givenName).font(.system(size: 15, weight: .semibold)); Text(w.label).font(.system(size: 12)).foregroundStyle(.secondary) }; Spacer(); Text("\(h.score)°").font(.system(size: 16, weight: .bold, design: .rounded)).foregroundStyle(w.color) }.padding(12).background(EchoTheme.cardGradient).clipShape(RoundedRectangle(cornerRadius: 12)) }
        }
    }
    private var weatherType: WeatherType { let atRisk = contacts.filter { AIEngine.healthScore(for: $0).level == .fading || AIEngine.healthScore(for: $0).level == .critical }.count; let r = contacts.isEmpty ? 0 : Double(atRisk) / Double(contacts.count); if r > 0.5 { return .hurricane }; if r > 0.3 { return .stormy }; if r > 0.15 { return .cloudy }; return .sunny }
    ...
    private var warningCount: Int { contacts.filter { AIEngine.healthScore(for: $0).level == .fading }.count }
    ...
    private func personalWeather(health: RelationshipHealth.HealthLevel, gap: Date) -> (icon: String, label: String, color: Color) {
        let days = Int(Date().timeIntervalSince(gap) / 86400)
        switch health { case .thriving: return ("sun.max.fill", "晴朗", Color(hex: "00D9A3")); case .stable: return ("cloud.sun.fill", "多云转晴", Color(hex: "00B4D8")); case .fading: return ("cloud.rain.fill", "小雨", Color(hex: "FFB347")); case .critical: return days > 60 ? ("tornado", "飓风", .red) : ("cloud.bolt.rain.fill", "雷暴", .red) }
    }
}
