...
struct SettingsView: View {
    @State private var showAbout = false; @State private var showPaywall = false
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: EchoTheme.spacing16) {
                        planCard.echoAppear(); accountCard.echoAppear(delay: 0.05)
                        VStack(spacing: EchoTheme.spacing4) { settingRow(icon: "link", label: "GitHub Repository", color: .blue) { if let url = URL(string: "https://github.com/bambi2008/echo") { UIApplication.shared.open(url) } }; settingRow(icon: "envelope", label: "Send Feedback", color: .orange) { if let url = URL(string: "mailto:bambi2008@users.noreply.github.com") { UIApplication.shared.open(url) } }; settingRow(icon: "info.circle", label: "About Echo", color: .gray) { showAbout = true } }.padding(EchoTheme.spacing8).background(EchoTheme.bgCard).clipShape(RoundedRectangle(cornerRadius: EchoTheme.radius16, style: .continuous)).echoAppear(delay: 0.1)
                        VStack(alignment: .leading, spacing: EchoTheme.spacing12) { HStack(spacing: EchoTheme.spacing8) { Image(systemName: "lock.fill").foregroundStyle(EchoTheme.success); Text("Your contacts stay on your device.").font(.subheadline).foregroundStyle(EchoTheme.textSecondary) }; HStack(spacing: EchoTheme.spacing8) { Image(systemName: "shield.fill").foregroundStyle(EchoTheme.success); Text("No cloud sync. No tracking. No ads.").font(.subheadline).foregroundStyle(EchoTheme.textSecondary) } }.padding(EchoTheme.spacing16).background(EchoTheme.bgCard).clipShape(RoundedRectangle(cornerRadius: EchoTheme.radius16, style: .continuous)).echoAppear(delay: 0.15)
            ...
            }.navigationTitle("Settings").navigationBarTitleDisplayMode(.large).alert("About Echo", isPresented: $showAbout) { Button("OK", role: .cancel) {} } message: { Text("Echo helps you stay in touch with the people who matter. All data stays on your device. v1.0") }.sheet(isPresented: $showPaywall) { PaywallView { } }
        }
    }
    private var planCard: some View {
        HStack(spacing: EchoTheme.spacing12) {
            ...
            VStack(alignment: .leading, spacing: 2) { Text(store.isPro ? "Echo Pro" : trial.isInTrial ? "Free Trial" : "Echo Personal").font(.headline).foregroundStyle(EchoTheme.textPrimary); if trial.isInTrial { Text("\(trial.trialDaysRemaining) days left in trial").font(.caption).foregroundStyle(EchoTheme.overdue) } else { Text(store.isPro ? "All features unlocked" : "Free · Limited features").font(.caption).foregroundStyle(EchoTheme.textTertiary) } }
            Spacer()
        ...
        }.padding(EchoTheme.spacing16).background(LinearGradient(colors: store.isPro ? [EchoTheme.accent.opacity(0.08), EchoTheme.bgCard] : trial.isInTrial ? [EchoTheme.overdue.opacity(0.08), EchoTheme.bgCard] : [EchoTheme.bgCard, EchoTheme.bgSecondary], startPoint: .topLeading, endPoint: .bottomTrailing)).clipShape(RoundedRectangle(cornerRadius: EchoTheme.radius16, style: .continuous)).overlay(RoundedRectangle(cornerRadius: EchoTheme.radius16, style: .continuous).stroke(store.isPro ? EchoTheme.accent.opacity(0.15) : trial.isInTrial ? EchoTheme.overdue.opacity(0.15) : Color.white.opacity(0.06), lineWidth: 0.5))
    }
