...
final class AIEngine {
 ...
 static func smartReminder(for c: EchoContact) -> SmartReminder? {
  ...
  let p: SmartReminder.ReminderPriority; let r: String
 ...
 static func generateInsights(for c: EchoContact) -> [AIInsight] {
  ...
  let txt = notes.map { $0.content }.joined(separator: " ")
 ...
 static func smartSearch(query: String, in contacts: [EchoContact]) -> [(EchoContact, String)] {
  ...
  for c in contacts { if c.fullName.lowercased().contains(q) { res.append((c, "名字匹配")); continue }; if let co = c.companyName?.lowercased(), co.contains(q) { res.append((c, "公司: \(c.companyName ?? "")")); continue }; for n in c.notes { if n.content.lowercased().contains(q) { res.append((c, "笔记: \(snippet(q: q, text: n.content))")); break } }; for i in c.interactions { let s = i.note.lowercased(); if s.contains(q) { res.append((c, "互动: \(snippet(q: q, text: i.note))")); break } } }
  return res
 }
 // 5. Goals
 ...
 static func generateOpeningLines(for c: EchoContact, channel: InteractionType) -> [AIOpeningLine] {
  ...
  let topics = extractTopics(txt: c.notes.map { $0.content }.joined(separator: " "))
 ...
 private static func calcSentiment(notes: [Note]) -> Double { guard !notes.isEmpty else { return 0.5 }; return notes.suffix(5).map { sentiment(txt: $0.content) }.reduce(0, +) / Double(min(notes.count, 5)) }
 ...
 private static func sentimentTrend(notes: [Note]) -> (Bool, Bool) { guard notes.count >= 2 else { return (false, false) }; let h = notes.count / 2; let f1 = notes.prefix(h).map { sentiment(txt: $0.content) }.reduce(0, +) / max(Double(h), 1); let f2 = notes.suffix(notes.count - h).map { sentiment(txt: $0.content) }.reduce(0, +) / max(Double(notes.count - h), 1); let d = f2 - f1; return (d < -0.15, d > 0.15) }
 private static func extractTopics(txt: String) -> [String] { guard !txt.isEmpty else { return [] }; let t = NLTagger(tagSchemes: [.nameType]); t.string = txt; var tp: [String] = []; t.enumerateTags(in: txt.startIndex..<txt.endIndex, unit: .word, scheme: .nameType, options: [.omitWhitespace]) { tag, r in if tag != nil { let w = String(txt[r]); if w.count >= 2 && !tp.contains(w) { tp.append(w) } }; return true }; return Array(tp.prefix(5)) }
 private static func detectActions(notes: [Note]) -> [String] { let p = ["记得","别忘了","需要","应该","计划","打算","TODO","待办","跟进","下次","答应","承诺","约","下周","明天","remind","should","plan to"]; var i: [String] = []; for n in notes { for s in n.content.components(separatedBy: CharacterSet(charactersIn: "。.！!？?\n")) { let t = s.trimmingCharacters(in: .whitespaces); if t.count >= 5 && p.contains(where: { t.lowercased().contains($0.lowercased()) }) { i.append(t) } } }; return i }
 private static func detectActions(from txt: String) -> [String] { detectActions(notes: [Note(content: txt)]) }
 private static func pattern(ints: [Interaction]) -> String { guard ints.count >= 3 else { return "" }; let tc = ints.reduce(into: [InteractionType: Int]()) { $0[InteractionType(rawValue: $1.type) ?? .messaged, default: 0] += 1 }; if let top = tc.max(by: { $0.value < $1.value }) { let s = ints.sorted { $0.date < $1.date }; let ag = s.count > 1 ? s.last!.date.timeIntervalSince(s.first!.date) / Double(s.count - 1) / 86400 : 0; return "你们最常通过「\(top.key.label)」联系，平均间隔 \(String(format: "%.1f", ag)) 天" }; return "" }
 ...
 private static func sentimentDetail(notes: [Note]) -> String { guard !notes.isEmpty else { return "暂无笔记" }; let a = notes.suffix(5).map { sentiment(txt: $0.content) }.reduce(0, +) / Double(min(notes.count, 5)); if a > 0.65 { return "笔记情感积极" }; if a > 0.45 { return "笔记情感中性" }; return "笔记情感偏消极" }
 private static func mostUsed(ints: [Interaction]) -> InteractionType { let c = ints.reduce(into: [InteractionType: Int]()) { $0[InteractionType(rawValue: $1.type) ?? .messaged, default: 0] += 1 }; return c.max(by: { $0.value < $1.value })?.key ?? .messaged }
